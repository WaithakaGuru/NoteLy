...
function MarkdownPreview(info: MarkdownContent) {
  return (
    ...
    >
      <Box component={"div"}>
        ...
        </Typography>
        {
          <Chip
            ...
            sx={{
              bgcolor: info.visibility ? "limegreen" : "slategrey",
              color: "#f9f9f9",
            }}
          />
        }
      ...
      <Markdown>{info?.state?.title || "No title written yet!!"}</Markdown>
      ...
      <Markdown>{info?.state?.synopsis || "No synopsis writen yet!!"}</Markdown>
      ...
      <Markdown>{info?.state?.content || "No content written yet !!"}</Markdown>
    </Box>
  );
}
