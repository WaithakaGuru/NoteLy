import { CalendarMonth, CalendarToday, Delete,  DeleteOutline, Edit, Menu, NoteAdd, Notes, Person, PushPin, Today, Topic, Visibility } from "@mui/icons-material"
import { Box, Button, Chip, Drawer, IconButton, Paper, Stack, TextField, Typography } from "@mui/material"
  ...
  return (
    ...
    >
      <Box component={"aside"} sx={{display: {xs: "none", sm:"flex"}}}>
        <Drawer open={isOpen} onClick={() =>setIsOpen(false)} sx={{
          '& .MuiDrawer-paper': {
            backgroundColor: '#011611',
            color: '#fff',
            width: "12rem",
      ...
      <Stack component={"section"} className="bg-gray-50 w-full h-[35rem] overflow-auto rounded-xl p-6"sx={{ml:{sm:"10rem"}}} >
        <IconButton sx={{bgcolor: "#dce6f6", '&:hover': {bgcolor: "#a9b6ca"}, 
        top:"4.4rem", left:".8rem", position: "absolute", display:{sm:"none",}}} 
         onClick={()=> setIsOpen(true)} className="w-11 h-11 z-10">
          <Menu className="text-purple-800"/> 
        </IconButton>
        <Box component={"section"} className="flex gap-4 border-b border-gray-300" sx={{flexDirection:{xs:"column", md: "row"}}}>
          <Stack direction={"row"} className="justify-between min-w-1/2">
            <Box>
              <Box component={"div"} className="flex items-center justify-around w-full">
                <Typography fontSize={"1.8rem"} fontWeight={"bold"} className="text-gray-700">
                    Dashboard
                </Typography>    
                <Typography fontWeight={600} fontSize={"1.1rem"}  className="text-purple-700 inline">
                  Hello Waithaka
                </Typography> 
              </Box>  
              ...
              <Stack direction={"row"} gap={1}>
                ...
                fontWeight: "bold", fontSize: "1.1rem"}} className="w-24 text-gray-50"
              >
                Notes
              </Button>
              </Stack>
       ...
       <Stack direction={"row"} className="justify-center gap-4 flex-wrap" >
          <Stack className="w-[30rem] p-4 items-left gap-4 shadow-xl min-h-84 border-gray-300 border rounded-xl justify-center">
           ...
           <Stack direction={"row"} className="gap-4 border-t border-gray-300 p-2 pt-8 items-center">
             ...
             <Button variant="contained" color="primary"sx={{textTransform: "none"}} startIcon={<Visibility/>} title="See full blog" href="/dashboard/note/1">View</Button> 
              <Button  color="secondary" startIcon={<Edit/>} sx={{bgcolor: "#f0e5ff", textTransform: "none"}} href="/dashboard/update/1" title="Edit this note">Edit</Button> 
              <button className="text-red-700 bg-red-200 p-1 rounded text-nowrap cursor-pointer" title="Delete this note" disabled={false}>
                <DeleteOutline/> Delete</button>
           </Stack>
          </Stack>
          <Stack className="w-[30rem] p-4 items-left gap-4 shadow-xl min-h-84 border-gray-300 border rounded-xl justify-center">
            <Typography variant="h5" className="text-gray-700 flex justify-between" fontWeight={600}>
          ...
          <Stack className="min-w-96  p-4 items-center gap-4 shadow-xl h-54 border-gray-300 border
            ...
            <Button variant="contained" color="secondary" title="Add a new title" href="/dashboard/create">Create a new note</Button>
          </Stack>
          <Stack className="min-w-96 p-4 items-center gap-4 shadow-xl h-54 border-gray-300 border rounded-xl justify-center">
            ...
            <Button variant="contained" color="primary" href="/dashboard" title="Go to Dashboard">Dashboard</Button>
          </Stack>
       </Stack>
      </Box>
      </Stack>
